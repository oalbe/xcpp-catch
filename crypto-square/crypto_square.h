...
class cipher {
    ...
    std::string normalize_text_helper() {
        ...
        // std::cout << "###### = " << lower(no_symbols) << '\n';
        __normalized_text = lower(no_symbols);
        // std::cout << "normalized1 = " << __normalized_text << '\n';
        return lower(no_symbols);
    }
public:
    cipher(std::string input): __plain_text(input), __normalized_text(normalize_text_helper()) {}
    ...
    std::string cipher_text() const {
        ...
        std::size_t plain_segments_len = plain_segments.size();
        std::size_t segment_len = plain_segments[0].size();
        for (std::size_t j = 0; j < segment_len; ++j) {
            for (std::size_t i = 0; i < plain_segments_len; ++i) {
